...
void Graphics::InitializeRenderingBuffers(uint32_t bufferWidth, uint32_t bufferHeight)
{
	...
	g_SceneColorBuffer.Init(L"Main Color Buffer", bufferWidth, bufferHeight, 1, DefaultHdrColorFormat);
	g_OverlayBuffer.Init(L"UI Overlay", g_DisplayWidth, g_DisplayHeight, 1, DXGI_FORMAT_R8G8B8A8_UNORM);
	g_HorizontalBuffer.Init(L"Bicubic Intermediate", g_DisplayWidth, bufferHeight, 1, DefaultHdrColorFormat);
...
{
	g_OverlayBuffer.Init(L"UI Overlay", g_DisplayWidth, g_DisplayHeight, 1, DXGI_FORMAT_R8G8B8A8_UNORM);
	g_HorizontalBuffer.Init(L"Bicubic Intermediate", g_DisplayWidth, NativeHeight, 1, DefaultHdrColorFormat);
}
