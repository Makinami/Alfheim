...
cbuffer fftBufer : register(b1)
{
	uint col_row_pass; // 0 - row, 1 - col
	float3 pad;
};

RWTexture2DArray<float4> Waves : register(u0);
...
void main( uint3 DTid : SV_DispatchThreadID )
{
	...
	pingPongArray[0][DTid.x] = Waves[uint3(texturePos, DTid.z)];
	...
	Waves[uint3(texturePos, DTid.z)] = pingPongArray[textureIndices.x][DTid.x];
}